...
pub struct Vertex {
    pub pos: [f32; 3],
    pub _pad: f32,       // 4-byte padding for 16-byte alignment (matches GlobalHeader.hlsli)
    pub color: [f32; 4],
}
...
pub struct AurenGraphicsPipeline {
...
impl Drop for AurenGraphicsPipeline {
    fn drop(&mut self) {
        unsafe {
